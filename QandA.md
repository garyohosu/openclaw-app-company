...
ステータス: 全50件 回答確定 / SPEC.md v0.7・USECASE.md 反映済み / Q51-Q54 未回答（SEQUENCE.md作成時追加）
...
## 2026-03-23 SEQUENCE.md 作成時の不明点（未回答）

---

## Q51. `run_phase()` からエージェントを呼ぶ方式は「関数呼び出し」か「subprocess」か

**該当箇所:** SEQUENCE.md 図2、SPEC.md Section 3.1

SPEC.md Section 3.1 の骨格コードでは `fn()` と書かれており、同一プロセス内での関数呼び出しに見える。
しかし、エージェントごとに Python 実行環境や依存ライブラリが異なる場合、`subprocess` で分離する設計のほうが安全なケースもある。

- 正式方式は同一プロセス内の `fn()` 関数呼び出しか
- それとも `subprocess.run(["python", "scripts/agents/xxx.py"])` の subprocess 方式か
- 将来的に subprocess に移行する可能性はあるか

シーケンス図の呼び出し矢印の意味（同期関数呼び出し vs プロセス起動）に影響する。

---

## Q52. フェーズ失敗時に `company_state.json` の `current_phase` は失敗フェーズのままか、前フェーズに戻るか

**該当箇所:** SEQUENCE.md 図1・図2、SPEC.md Section 15（フェーズ失敗時の差し戻し方式）、Section 21（状態管理）

SPEC.md には「前フェーズの成果物は保持する / 修正後に同一フェーズを再実行する」と書かれているが、
`current_phase` の値が失敗時にどう変化するかが明記されていない。

- 失敗時は `current_phase` を失敗したフェーズの識別子のままにするか
- 前フェーズの識別子に戻すか
- `next_action` と組み合わせて「失敗中」状態を表現するか

シーケンス図の状態遷移の正確な描き方に影響する。

---

## Q53. Phase 11 の git push は運営者が手動でターミナルから実行するか、それとも `scripts/main.py` が承認後に自動で実行するか

**該当箇所:** SEQUENCE.md 図10、SPEC.md Section 15（`git push` は人間承認付き操作）

SPEC.md には「`git push`・GitHub Pages 反映・リリース扱いの更新は人間承認付き操作とする」と書かれているが、
具体的な操作手順が不明。

- 運営者がターミナルで `git push origin main` を直接入力するか
- `scripts/main.py` が承認プロンプトを出し、"yes" 入力後に `subprocess` で `git push` を自動実行するか
- リリース用の別スクリプト（`scripts/release.py` 等）を用意するか

シーケンス図での「誰が git push を実行するか」に直接影響する。

---

## Q54. `visitor.cgi` への自動記録は「ページロード時に app.js が fetch する」か「別のタイミングか」

**該当箇所:** SEQUENCE.md 図11、SPEC.md Section 9.5（visitor.cgi）

SEQUENCE.md の図11 では「ページロード時に app.js が自動で fetch する」と描いたが、
SPEC.md の Section 9.5 には自動記録のタイミングが明記されていない。

- ページロード時（`DOMContentLoaded` または `window.onload`）に自動 fetch するか
- 何らかのユーザー操作（初回インタラクション等）をトリガーにするか
- `create-app-template.py` が生成するテンプレートに visitor.cgi 呼び出しコードを含めるか

SPEC.md Section 9.5 または Section 22 の `spec.md` テンプレートに記載すべきかの確認が必要。
