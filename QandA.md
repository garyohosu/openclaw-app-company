...
ステータス: 全64件 回答確定 / SPEC.md v0.9 反映済み
...
## 2026-03-23 CLASS.md / 関連md 再レビュー時の不明点（Q58〜Q60 回答確定）

---

## Q58. 各アプリ `spec.md` の `cors_origin` は単一文字列か、複数許可オリジンの配列か

**回答:**
`spec.md` では `cors_origins: list[str]` を正式仕様とする。単数の `cors_origin` は廃止する。1件のみでも配列で表現する。

**正式方針:**
- フィールド名は `cors_origins`
- 型は文字列配列
- 1件しかない場合も配列で表す
- 例: `["https://garyohosu.github.io", "http://localhost:8000"]`

**理由:**
CORS 方針は許可オリジンを限定することが目的であり、GitHub Pages 本番 URL とローカル確認 URL を併用しうるため、単数フィールドでは表現力が不足する。SPEC のセキュリティ方針とも複数オリジン許可の実態とも整合させるため、配列を正本にする。

---

## Q59. CLASS.md の `AdSenseStatus` は残すか、それとも `adsense_*` フラグ群に一本化するか

**回答:**
`AdSenseStatus` は削除し、`adsense_*` フラグ群に一本化する。

**正式方針:**
- AdSense 判定の正本は以下の項目とする
- `adsense_required`
- `adsense_applied`
- `adsense_checked_at_release`
- `adsense_exception_reason`
- `AdSenseStatus` は MVP の実装・仕様上の正本にしない

**理由:**
SPEC Section 22 はすでにフラグ群でテンプレート項目を定義している。ここに別の列挙状態を加えると二重管理になり、設計図と実装仕様の対応が悪くなるため。

---

## Q60. `DeployReport` の「一覧ページ反映状況」は独立フィールドとして固定するか

**回答:**
独立フィールドとして固定する。`DeployReport` に `index_updated: bool` を追加する。

**理由:**
公開判定では「一覧ページから辿れること」「ルート一覧も更新すること」が明示されており、`deploy_report.md` は人間向け監査記録の正本である。したがって、一覧反映状況は自由記述ではなく明示フィールドで持つほうが判定・監査に適している。

---

## 2026-03-23 TEST.md レビュー時の不明点（Q61〜Q64 回答確定）

---

## Q61. TDD の標準テストフレームワークと配置は `pytest` / `tests/` に固定してよいか

**回答:**
固定してよい。MVP の標準テストフレームワークは `pytest`、テスト配置は `tests/` とする。`tmp_path` フィクスチャを使った一時ディレクトリ前提も正式仕様に含める。`tests/` ディレクトリは Phase 0 の正式成果物に追加する。

**理由:**
`TEST.md` はすでに `tmp_path` ベースのテスト設計になっており、フレームワーク未確定のままだと受け入れ条件として弱い。MVP では `pytest` が最も自然で、実装者の迷いも少ない。

---

## Q62. `CompanyState.load(path)` のファイル不在時は例外送出か、初期状態を返すか

**回答:**
`FileNotFoundError` を送出する。`CompanyState.load(path)` は既存状態の読み込み専用とし、ファイル不在時に黙って初期状態を返さない。初期状態の生成は OpenClaw 側の初期化処理、または `CompanyState.default()` のような別APIで明示的に扱う。

**理由:**
初回起動と異常な状態欠損を区別しやすくなり、`load()` の責務が明確になる。受け入れテストも曖昧にならない。

---

## Q63. TDD の自動テストで Phase 6 / 7 の CGI 疎通をどこまで実サーバー接続するか

**回答:**
MVP の自動テストでは外部 CGI をモックに固定し、実サーバーへの疎通確認は手動確認とする。Phase 6 / 7 の `pytest` では、リクエスト生成、レスポンス解釈、レポート生成、成功/失敗分岐までを自動検証対象にする。実際の Sakura サーバー到達性、SSL、CORS、実DB応答は手動の結合確認で扱う。

**正式境界:**
- 自動テストに含めるもの
- 採用 CGI 一覧の確認ロジック
- モック応答に基づく `api_connectivity_report.md` / `db_connectivity_report.md` 生成
- `SystemExit(0/1)` の分岐
- OpenClaw による state 更新統合テスト
- 手動確認に残すもの
- 実URL到達性
- SSL 実確認
- CORS 実確認
- `db.cgi` の本番/検証サーバー応答確認

**理由:**
`browser-use CLI` や手動確認はブラウザ確認の正式手段として別に残す。MVP は「自動はモック、実疎通は手動」の二段構えでよい。

---

## Q64. Pages E2E テストの公開URLは固定文字列か、`pages_path` / `deploy_report.md` 由来か

**回答:**
Pages E2E の公開URLの正本は、公開後は `DeployReport.public_url`、公開前の構成確認は `AppSpec.pages_path` とする。固定文字列の GitHub Pages URL を `TEST.md` に直接埋め込まない。

**正式方針:**
- 公開前テスト: `pages_path` を使って相対パス・生成先を確認
- 公開後 E2E: `deploy_report.md` に記録された `public_url` を使って確認

**理由:**
SPEC は公開URLを `deploy_report.md` の正式記録項目として扱っており、CLASS.md にも `public_url` が定義されている。運用変更やリポジトリ名変更に強くするためにも固定URL埋め込みは避ける。

---

## 2026-03-23 ソースコードレビューでの不明点（Q65〜Q68）

## Q65. `scripts/main.py` の `--phase` / `--dry-run` はどこまで実装対象か

**質問:**
CLI オプションは定義されていますが実装が未反映です。`--phase` は特定フェーズのみ実行する機能として必須か、MVPでは未使用でよいか。`--dry-run` は Git 操作の抑制だけでなくファイル生成や state 更新まで抑制するか、どこまで抑制するかを明確化したいです。
...
## Q66. `quality_gate` の更新はフェーズ失敗時・レポート不在時にどう扱うか

**質問:**
`update_state_after_phase()` は成功時に `true` へ更新するのみで、失敗時やレポート不在時に `false` へ戻しません。再実行時の「前回の true が残る」挙動を許容するか、フェーズ開始時に対象フラグをリセットするかの方針が必要です。
...
## Q67. `spec_validator` のフィールド抽出は YAML 風のインデントを許容すべきか

**質問:**
`_extract_field()` は行頭一致のみで、先頭にスペースがあると検出できません。`spec.md` の運用でインデント・Front Matter・コードブロック内の表記を許容するなら正規表現の緩和が必要です。想定フォーマットを明文化したいです。
...
## Q68. `CompanyState.current_phase` / `current_sprint` の更新責務は `main.py` で持つか

**質問:**
現在の実装ではフェーズ進行中に `current_phase` や `current_sprint` を更新していません。Phase 開始時・完了時に更新するのか、もしくは別エージェントが更新するのか責務を明確化したいです。
...
**回答（確定）:**
- どちらも MVP 必須。
- `--phase` は単一フェーズ実行に限定し、指定フェーズのみを前提入力確認の上で実行する。前後フェーズは自動実行しない。実行後は `current_phase` / `next_action` / `quality_gate` を通常どおり更新する。
- `--dry-run` は完全な no-write モード。以下を行わない:
  - ファイル新規作成・上書き
  - `state/company_state.json` 更新
  ...
  - 実行対象エージェント一覧表示
  - 期待出力パス表示
  - 予定 `quality_gate` 更新内容の表示

---

**回答（確定）:**
- 対象フェーズ開始時に、そのフェーズが責任を持つ `quality_gate` 項目を一旦 `false` にリセットする。
- 「フェーズ成功 + 必須レポート存在 + 合格条件を満たす」の3条件を満たす場合のみ `true` に更新する。
- 失敗時は `false` のまま保持。`current_phase` は失敗したフェーズを保持し、`next_action` に修正指示を記録する。

---

**回答（確定）:**
- `spec.md` の正本は Markdown テンプレート項目とする。Front Matter は MVP 非対応。
- `_extract_field()` は行頭空白を許容する程度に緩和する。
- コードブロック内の記述は抽出対象外とする。

---

**回答（確定）:**
- `current_phase` / `current_sprint` の更新責務は `scripts/main.py` に置く。
- フェーズ開始時に `current_phase` を対象フェーズへ更新する。
- 成功時もそのフェーズを保持し、次の動きは `next_action` で表現する。
- 失敗時は失敗したフェーズを保持し、`next_action` に修正指示を記録する。
