...
- ドキュメント版数: v0.2
- 作成日: 2026-03-23
- 最終更新: 2026-03-23（QandA.md 回答内容を反映）
...
OpenClaw は本プロジェクトにおける **オーケストレーション実行基盤** の総称である。外部製品名ではなく、以下の役割を担う仕組みを指す。

- エージェント定義の読み込み（`agents/agents.yaml`）
- フェーズ進行管理
- 成果物ファイルの受け渡し
- Codex CLI 実行指示の発行
- 品質ゲート判定
...
環境は `production` / `local-test` の2系統とする。GitHub Pages 本番から呼ぶ URL は本番固定。開発時は `test-api.html` で個別確認する。
