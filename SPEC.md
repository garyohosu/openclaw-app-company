...
GitHub Pages で**公開する全HTMLページ**には Google AdSense の共通タグを `<head>` 内へ挿入する。対象は少なくともルート `index.html`、各アプリの `index.html`、`test-api.html`、公開用 `test-db-readonly.html` とする。ローカル専用確認ページは対象外。
...
`current_phase` は英語識別子で持つ。正規の enum は `initialization` `research` `idea_selection` `prd` `design` `task_breakdown` `api_connectivity` `db_connectivity` `implementation` `testing` `release` `improvement` とする。詳細なレビューステップを区別したい場合は `sub_phase` または `next_action` で補助管理する。
...
- `scripts/create-app-template.py` を作成する
- `scripts/build-index.py` を作成する
